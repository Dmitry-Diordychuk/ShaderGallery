Shader "Unlit/Test" // путь для инспектора материала и имя шейдера
{
    ...
    SubShader // может быть несколько SubShader-ов будет выбран тот чье возможности наиболее подходят для текущего устройства. Например: IOS библиотека Metal
    {
        ...
        /* AlphaTest мужду геометрией и прозрачными объектами. Transparent прозрачные объекты. Overlay UI*/
            "Queue"="Geometry" /* по умолчанию в коде не пишется. Связан с камерой                       */
        /* ??? Связан с replacement через скрипт замена шейдера                                          */
            "RenderType"="Opaque" /* Opaque по умолчанию                                                 */
            //"RenderPipeline"="UniversalRenderPipeline" /* поддержка URP РАБОТАЕТ ТОЛЬКО С HLSL         */             (7)
        } /*                                                                                             */
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        {
            // Cull можно написать здесь аналогично Tags                                                                (3)
            ...
            // fixed - 11 bit. Простые операции. Например с цветами. Не работает в HLSL                                 (7)
            //
            ...
            //#include "Package/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl" Все функции из библиотеки выше не будут работать (7)
            ...
            {
                float2 uv : TEXCOORD0;
                UNITY_FOG_COORDS(1) // Перестанет работать в HLSL                                                         (6)(7)
            ...
            // Возвращает значение перез аргументы
            void FakeLight2(in float3 Normal, out float3 Out)
            {
                float3 operation = Normal;
                Out = operation;
            }
            ...
            {
                v2f o;
                o.vertex = UnityObjectToClipPos(v.vertex); // Перестанет работать в HLSL вместо неё TransformObjectToHClip (6)(7)
                o.uv = TRANSFORM_TEX(v.uv, _MainTex); // Перестанет работать в HLSL                                        (6)(7)
            ...
            // half4 если HLSL       SV_Target (System Value Target) output который позволяет рендерить в промежуточный буффер (render target)
            {
                ...
                // apply fog
                UNITY_APPLY_FOG(i.fogCoord, col); // Перестанет работать в HLSL                                         (6)(7)
